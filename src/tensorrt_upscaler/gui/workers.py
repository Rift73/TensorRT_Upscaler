...
def create_upscaler(cfg, onnx_path: str) -> ImageUpscaler:
    """Create an ImageUpscaler from config. Shared by UpscaleWorker and ClipboardWorker."""
...
class UpscaleWorker(QThread):
    ...
    def _process_animated_image(self, input_path, output_path, upscaler, cfg):
        """Process an animated image (GIF, animated WebP, APNG)."""
        ...
        animated_upscaler.upscale_animated(
            ...
            prescale_scale_factor=cfg.prescale_scale_factor,
            ...
            custom_res_scale_factor=cfg.custom_res_scale_factor,
    ...
    def _process_static_image(self, img, img_has_alpha, upscaler, cfg):
        """Apply pre-scale, upscale, custom res, and sharpening to a static image."""
        ...
        if cfg.custom_res_enabled:
            new_size = compute_scaled_size(
                width, height,
                cfg.custom_res_mode, cfg.custom_res_width, cfg.custom_res_height,
    ...
    def run(self):
        try:
            ...
            if cfg.upscale_enabled:
                upscaler = create_upscaler(cfg, self.onnx_path)
            ...
            try:
                ...
                for i, input_path in enumerate(self.files):
                    ...
                    # Generate output path (needed for skip check)
                    ...
                    if is_animated(input_path):
                        output_path = self._process_animated_image(
                            input_path, output_path, upscaler, cfg
                        )
                    ...
                    # Process: pre-scale, upscale, custom res, sharpen
                    img = self._process_static_image(img, img_has_alpha, upscaler, cfg)

                    # Save as PNG asynchronously
                    async_saver.save(img, output_path, img_has_alpha, img_metadata)

                    # PNG optimization (must wait for save to complete)
                    ...
                    if cfg.secondary_enabled:
                        sec_size = compute_scaled_size(
                            ...
                            scale_factor=cfg.secondary_scale_factor,
                        )
